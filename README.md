# Cipher
Cipher is a game made for a computer and possibly a modded Wii U. It follows the story of Mello, who fell off a cruise and turned into an eel (crazy, I know bro, but stick with me).
## Features
* Cipher has multiple perspectives for each chapter. At the start of each chapter, the player will choose what available character to play as.
* When you complete the game, you will be able to find CDs explaining the story.
* Features 3D low-poly graphics.
* Both screens are used.
## Development
Development is currently in process. Contributing is absolutely okay, and I will probably accept it. Please read the contribution guidelines before doing so.
### Current available positions for development:
* Music designer
* Programmer
* Modeler
* Rigger
* Playtesters
* Sound designer
## .wuhb download
.wuhb downloads will be available once chapters are released.
# FAQ
## When will the first chapter be made?
I expect it to be about June, 2026.
## Where can I contact you?
Contact me at flakeorg@gmail.com
## I want to make a mod. Can I do so?
Yes, but under the following circumstances:

* You do not monetize the mod
* You keep everything age appropriate (maximum PG-13)
* The mod is not intended to cause hate on another individual
## Can I use your code/assets to build my game?
Not without permission. Contact me first.
## Will there be voice acting?
No. This is because I cannot afford to hire people, and the file size needs to be managable.
## Can I create fan art?
Yes, with the same rules as for modding.
## I want to make content on Cipher.
You may, please read acceptable and unacceptable use of making content.
### Acceptable:
* Fan animations
* Let's play videos
* Reviews
* News about the game
* Fan art
* Lore videos
### Not Acceptable:
* Animated series
* Graphic content
* Content that alters the story to the extent where the story is meant to harm others
* Creating Merch
* Content intended to dox others.
<br>
Please note that I can still take your content down by discretion. Use common sense when making content. Contact me for questions.

## Will this be released on other platforms?
Yes, but please note that the ports may be paid.
